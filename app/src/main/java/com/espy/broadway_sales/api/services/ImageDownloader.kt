...
class ImageDownloader {
    ...
    fun downloadImage2Async(context: Context, url: String, fileName: String): Deferred<String> {
        return GlobalScope.async {
            val mydir = File(context.getExternalFilesDir(null).toString() + "/broadway_salesFiles")
            if (!mydir.exists()) {
                mydir.mkdirs();
            }
            ...
            request.setAllowedNetworkTypes(
                ...
                .setDestinationInExternalFilesDir(context, "/broadway_salesFiles", fileName)
    ...
    fun downloadImage3Async(context: Context, url: String, fileName: String): Deferred<String> {
        return GlobalScope.async {
            ...
            try {
                ...
                val file = File(context.getExternalFilesDir(null).toString() + "/broadway_salesFiles/", fileName)

                response.body?.let {body ->
                    val desDir = File(context.getExternalFilesDir(null).toString() + "/broadway_salesFiles/")
    ...
    fun downloadImageAsync(context: Context, url: String, fileName: String, storageName: String): Deferred<String> {
        return GlobalScope.async {
            ...
            try {
                var pathFolder = context.getExternalFilesDir(null).toString() + "/broadway_salesFiles/"
